...
class TileCreator(object):
    ...
    def __init__(self, tileSizeLow, simSizeLow=64, upres=2, dim=2, dim_t=1, overlapping=0, densityMinimum=0.02,
        ...
        if np.less(self.simSizeLow, self.tileSizeLow).any():
            self.TCError(
                'Tile size {} can not be larger than sim size {}, {}.'.format(self.tileSizeLow, self.simSizeLow))
        ...
        self.densityThreshold = (
                self.densityMinimum * self.tile_shape_low[0] * self.tile_shape_low[1] * self.tile_shape_low[2])
    ...
    def initDataAugmentation(self, rot=2, minScale=0.85, maxScale=1.15, flip=True):
        ...
        if rot == 2:
            ...
            self.cube_rot = {2: [[], [z], [z, z], [nz]],
                             3: [[], [x], [y], [x, x], [x, y], [y, x], [y, y], [nx], [x, x, y], [x, y, x], [x, y, y],
                                 [y, x, x], [y, y, x], [ny], [nx, y], [x, x, y, x], [x, x, y, y], [x, y, x, x], [x, ny],
                                 [y, nx], [ny, x], [nx, y, x], [x, y, nx], [x, ny, x]]}
        else:
            self.do_rotation = False
            self.do_rot90 = False
    ...
    def addData(self, low, high):
        ...
        if not self.data_flags[DATA_KEY_HIGH]['isLabel']:
            if len(low.shape) != len(high.shape):  # high-low mismatch
                self.TCError(
                    'Data shape mismatch. Dimensions: {} low vs {} high. Dimensions must match or use highIsLabel.'.format(
                        len(low.shape), len(high.shape)))
        if not (len(low.shape) == 4 or len(low.shape) == 5):  # not single frame or sequence of frames
            self.TCError(
                'Input must be single 3D data or sequence of 3D data. Format: ([batch,] z, y, x, channels). For 2D use z=1.')

        if (low.shape[-1] != (self.dim_t * self.data_flags[DATA_KEY_LOW]['channels'])):
            self.TCError(
                'Dim_t ({}) * Channels ({}, {}) configured for LOW-res data don\'t match channels ({}) of input data.'.format(
                    self.dim_t, self.data_flags[DATA_KEY_LOW]['channels'], self.c_low, low.shape[-1]))
        if not self.data_flags[DATA_KEY_HIGH]['isLabel']:
            if (high.shape[-1] != (self.dim_t * self.data_flags[DATA_KEY_HIGH]['channels'])):
                self.TCError(
                    'Dim_t ({}) * Channels ({}, {}) configured for HIGH-res data don\'t match channels ({}) of input data.'.format(
                        self.dim_t, self.data_flags[DATA_KEY_HIGH]['channels'], self.c_high, high.shape[-1]))
        ...
        if self.premadeTiles:
            ...
            if not np.array_equal(low_shape, self.tile_shape_low) or not np.array_equal(high_shape,
                                                                                        self.tile_shape_high):
                self.TCError('Tile shape mismatch: is - specified\n\tlow: {} - {}\n\thigh {} - {}'.format(low_shape,
                                                                                                          self.tile_shape_low,
                                                                                                          high_shape,
                                                                                                          self.tile_shape_high))
        else:
            ...
            if not np.array_equal(single_frame_low_shape, self.frame_shape_low) or not np.array_equal(
                    single_frame_high_shape, self.frame_shape_high):
                self.TCError(
                    'Frame shape mismatch: is - specified\n\tlow: {} - {}\n\thigh: {} - {}, given dim_t as {}'.format(
    ...
    def createTiles(self, data, tileShape, strides=-1):
        ...
        noTiles = [(dataShape[0] - tileShape[0]) // strides[0] + 1, (dataShape[1] - tileShape[1]) // strides[1] + 1,
                   (dataShape[2] - tileShape[2]) // strides[2] + 1]
        tiles = []
    ...
    def cutTile(self, data, tileShape, offset=[0, 0, 0]):
        ...
        if np.less(data.shape[:3], tileShape[:3] + offset[:3]).any():
            self.TCError('Can\'t cut tile with shape {} and offset{} from data with shape {}.'.format(tileShape, offset,
                                                                                                      data.shape))

        tile = data[offset[0]:offset[0] + tileShape[0], offset[1]:offset[1] + tileShape[1],
               offset[2]:offset[2] + tileShape[2], :]
    ...
    def generateTile(self, isTraining=True, tile_t=1):
        ...
        if not self.premadeTiles:
            ...
            if self.do_scaling or self.do_rotation:
                ...
                if self.dim == 2:
                    tileShapeLow[0] = 1
                data[DATA_KEY_LOW], data[DATA_KEY_HIGH] = self.getRandomTile(data[DATA_KEY_LOW], data[DATA_KEY_HIGH],
                                                                             tileShapeLow.astype(int))
            ...
            # get a tile
            data[DATA_KEY_LOW], data[DATA_KEY_HIGH] = self.getRandomTile(data[DATA_KEY_LOW], data[DATA_KEY_HIGH],
                                                                         bounds=bounds)  # includes "shifting"
        ...
        if not np.array_equal(data[DATA_KEY_LOW].shape, target_shape_low) or (
                not np.array_equal(data[DATA_KEY_HIGH].shape, target_shape_high) and not self.data_flags[DATA_KEY_HIGH][
            'isLabel']):
            self.TCError(
                'Wrong tile shape after data augmentation. is: {},{}. goal: {},{}.'.format(data[DATA_KEY_LOW].shape,
                                                                                           data[DATA_KEY_HIGH].shape,
                                                                                           target_shape_low,
                                                                                           target_shape_high))
    ...
    def getDatum(self, index, tile_t=1):
        ...
        if not self.data_flags[DATA_KEY_HIGH]['isLabel']:
            return np.copy(self.data[DATA_KEY_LOW][index // self.dim_t][:, :, :, begin_ch:end_ch]), np.copy(
                self.data[DATA_KEY_HIGH][index // self.dim_t][:, :, :, begin_ch_y:end_c_h_y])
        else:
            return np.copy(self.data[DATA_KEY_LOW][index // self.dim_t][:, :, :, begin_ch:end_ch]), np.copy(
                self.data[DATA_KEY_HIGH][index // self.dim_t])
    ...
    def scale(self, data, factor):
        ...
        for data_key in data:
            if not self.data_flags[data_key]['isLabel']:
                data[data_key] = scipy.ndimage.zoom(data[data_key], scale, order=self.interpolation_order,
                                                    mode=self.fill_mode, cval=0.0)
    ...
    def parseChannels(self, channelString):
        ...
        c_types = {
            ...
            C_KEY_VELOCITY: [],
            # list of ordered (x,y,z; chnage to z,y,x to match data?) triples of velocity sets. e.g. for normal sim data [[1,2,3]]
            C_KEY_VORTICITY: []
        }
    ...
    def parseCVector(self, c, i, c_types, c_key, c_name='vector'):
        ...
        if c[i][-1] == 'x' or c[i][-1] == 'y' or c[i][-1] == 'z':
            ...
            if c.count(v_x) > 1:
                self.TCError(
                    'Duplicate {} ({}) x-channel with label \"{}\": {}. Vector keys must be unique.'.format(c_name,
                                                                                                            c_key,
                                                                                                            label, v_x))
            if c.count(v_y) > 1:
                self.TCError(
                    'Duplicate {} ({}) y-channel with label \"{}\": {}. Vector keys must be unique.'.format(c_name,
                                                                                                            c_key,
                                                                                                            label, v_y))
            if c.count(v_z) > 1:
                self.TCError(
                    'Duplicate {} ({}) z-channel with label \"{}\": {}. Vector keys must be unique.'.format(c_name,
                                                                                                            c_key,
                                                                                                            label, v_z))
        ...
        # check wrong suffix
            ...
            self.TCError(
                'Channel {}, \"{}\": unknown {} ({}) channel suffix \"{}\". Valid suffixes are \"x\", \"y\", \"z\".'.format(
                    i, c[i], c_name, c_key, c[i][-1]))
...
def savePngsBatch(low, high, TC, path, batchCounter=-1, save_vels=False, dscale=1., vscale=1.):
    ...
    for axis in range(1 if dim == 2 else 3):
        ...
        savePngsGrayscale(tiles=highD, path=path + 'high_' + suff, imageCounter=0,
                          tiles_in_image=[batch // tileX, tileX], channels=chout)

    # plot velocities , for individual samples
    if save_vels:
        for i in range(low.shape[0]):
            saveVelChannels(low[i], TC.c_lists[DATA_KEY_LOW][C_KEY_VELOCITY], path=path + 'low_vel_i{:02d}_'.format(i),
                            name="", scale=vscale)
        for i in range(high.shape[0]):
            saveVelChannels(high[i], TC.c_lists[DATA_KEY_HIGH][C_KEY_VELOCITY],
                            path=path + 'high_vel_i{:02d}_'.format(i), name="", scale=vscale)


# simpler function to output multiple tiles into grayscale pngs
def savePngsGrayscale(tiles, path, imageCounter=0, tiles_in_image=[1, 1], channels=[0], save_gif=False,
                      plot_vel_x_y=False, save_rgb=None, rgb_interval=[-1, 1]):
    '''
        tiles_in_image: (y,x)
    ...
    for image in range(noImages):
        ...
        if len(img_c) > 1 and (plot_vel_x_y or save_rgb != None):
            ...
            if save_rgb != None: saveRGBChannels(img, path, save_rgb, value_interval=rgb_interval,
                                                 imageCounter=imageCounter + image)

        import cv2
        if len(channels) == 1:
            data = np.uint8(np.clip(img_c[channels[0]] * 255, 0, 255))
            data = cv2.flip(data, 0)
            # print(img_c[channels[0]].shape)
            cv2.imwrite(path + "channel1_%04d.jpg" % (imageCounter * noImages + image), data)
        else:
            for i in channels:
                data = np.uint8(np.clip(img_c[i] * 255, 0, 255))
                data = cv2.flip(data, 0)
                cv2.imwrite(path + "notchannel1_%04d" % (imageCounter * noImages + image) + "_%04d.jpg" % (i), data)
...
def saveRGBChannels(data, path, channel_list, imageCounter=0, value_interval=[-1, 1]):
    ...
    import cv2
    data = np.uint8(np.clip(img * 255, 0, 255))
    data = cv2.flip(data, 0)
    cv2.imwrite(path + "rgbchannel%04d.jpg" % (imageCounter), data)
...
def save3DasUni(tiles, path, motherUniPath, imageCounter=0, tiles_in_image=[1, 1]):
    ...
    img_all = np.array(img_all)
    TDarrayToUni(img_all, path + 'source_{:04d}.uni'.format(imageCounter), motherUniPath, img_all.shape[0],
                 img_all.shape[1], img_all.shape[2])
...
def gridInterpolBatch(macgridbatch, targetshape, order=1):
    ...
    mactargetbatch = scipy.ndimage.map_coordinates(macgridbatch, [b, z * fz, y * fy, x * fx, c], order=order,
                                                   mode='reflect')
...
def selectRandomTempoTiles(self, selectionSize, isTraining=True, augment=False, n_t=3, dt=0.5, adv_flag=1.0):
    ...
    ori_input_shape = batch_low.reshape(
        (batch_sz, self.tileSizeLow[0], self.tileSizeLow[1], self.tileSizeLow[2], n_t, -1))
    ori_input_shape = np.transpose(ori_input_shape, (0, 4, 1, 2, 3, 5))
    ori_input_shape = ori_input_shape.reshape(
        (real_batch_sz, self.tileSizeLow[0], self.tileSizeLow[1], self.tileSizeLow[2], -1))
    ...
    if adv_flag:
        ...
        macgrid_input = macgrid_input.reshape(
            (real_batch_sz, self.tileSizeLow[0], self.tileSizeLow[1], self.tileSizeLow[2], 3))
        ...
        vel_pos_high_inter = getSemiLagrPosBatch(macgrid_input, dtArray, self.tileSizeHigh[1]).reshape(
            (real_batch_sz, -1))

    # return reshape_input, selectedOutputs, vel_pos_high_inter
    batch_high = batch_high.reshape(
        (batch_sz, self.tileSizeHigh[0], self.tileSizeHigh[1], self.tileSizeHigh[2], n_t, -1))
