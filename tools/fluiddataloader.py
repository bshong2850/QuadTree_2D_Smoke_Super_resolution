...
class FluidDataLoader(object):
	...
	def __init__(self, print_info=1, base_path="../data/", simdirname="", indices=[1],
	...
	def getFilename(self, fnbase, frame_index):
		if not self.oldNamingScheme:
			fn = os.path.join( self.base_path,(fnbase % frame_index))
		else:
			# both parts simdir & file have both indices!
			fn = os.path.join( self.base_path, (fnbase % (sim_index,frame_index)))
		return fn
	...
	def collectFilenamesFromDir(self, list_index): 
		...
		if self.wildcard is not None:
			...
			for t in range(0,n):
				...
				fn = self.getFilename(self.filename, filelist_index)
				self.xfn.append(fn)
				foundCnt += 1

				if self.filename_y is not None:
					fny = self.getFilename(self.filename_y, filelist_index)
					self.yfn.append(fny)
					self.have_y_npz = True # flag to indicate we have np arrays in y
